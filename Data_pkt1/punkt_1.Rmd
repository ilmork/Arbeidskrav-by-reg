...
Nas_arbeid <- Nas_arbeid_norge_sys %>% 
  mutate(knavn = Nas_arbeid_raw$region)
```
...
# Gir antall ansatte i de tre alderskategoriene i de ulike næringene, Haugalandet
...
```{r}
names(Haug_arbeid_raw2)[1]<-"desc9"
```

```{r}
Haug_arbeid_2 <- unlist(
  Haug_arbeid_raw2, 
  recursive = FALSE
) %>% 
  as_tibble() %>% 
...
```{r}
# Gir antall ansatte i de tre alderskategoriene i de ulike næringene på landsbasis
Haug_arbeid_wide <- Haug_arbeid_2 %>% 
  pivot_wider(
    id_cols = Aar,
    names_from = c(SN2007, alder),
    values_from = ansatte
...
```
%>% %>% %>% %>% %>% %>% 
# Gir antall ansatte i de tre alderskategoriene i de ulike næringene, Sunnhordland

```{r}
knr_sunn1 <- c(knr_sunn <- c("4611", "4612",
         "4613", "4614", "4615", "4616", "4617", "4618"))
```

```{r}
Sunn_arbeid_raw2 <- ApiData(
  urlToData = "07984",
  Region = list("vs:KommunerS", knr_sunn1),
  NACE2007 = TRUE,
  Kjonn = "0",
  ContentsCode = "SysselsatteArb",
  Tid = c(as.character(2008:2020)),
...
```{r}
names(Sunn_arbeid_raw2)[1]<-"desc10"
```

```{r}
Sunn_arbeid_2 <- unlist(
  Sunn_arbeid_raw2, 
  recursive = FALSE
) %>% 
  as_tibble() %>% 
...
```{r}
# Gir antall ansatte i de tre alderskategoriene i de ulike næringene på landsbasis
Sunn_arbeid_wide <- Sunn_arbeid_2 %>% 
  pivot_wider(
    id_cols = Aar,
    names_from = c(SN2007, alder),
    values_from = ansatte
...
write_csv(Haug_arbeid_wide, "Haug_arbeid_wide.csv")
write_csv(Haug_arbeid_2, "Haug_arbeid_2.csv")
write_csv(Sunn_arbeid_2, "Sunn_arbeid_2.csv")
write_csv(Sunn_arbeid_wide, "Sunn_arbeid_wide.csv")
show_col_type = FALSE
```
