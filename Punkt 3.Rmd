...
```{r}
Haug_sunn_first = left_join(Haug_bo, Sunn_bo)
```

```{r}
Haug_sunn = mutate(Haug_sunn_first, tot = tot_h+tot_s)
```

```{r}
syssel_bo_raw <- read.csv("Data_pkt1/Nas_arbeid_norge.csv")
```

```{r}
syssel_bo <- syssel_bo_raw %>% 
  select(ansatte)
```


```{r}
Haug_sunn <- merge(Haug_sunn, syssel_bo)
```
...
```{r}
 Haug_sunn%>% 
  ggplot(mapping = aes(x= ansatte, y = Aar)) +
  geom_line()+
