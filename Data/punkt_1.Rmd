...
Nas_arbeid_raw <- ApiData(
  ...
  # Neste hvis dere virkelig ønsker tre kategorier
...
```{r}
# Nytt navn første element i listen Nas_arbeid_raw
names(Nas_arbeid_raw)[1]<-"desc6"
```

```{r}
# Hele landet, Knr, knavn ikke relevant
# Har tre alt. for ansatte 15 - 74 år, 55-66 år og 67 - 74 år
Nas_arbeid_norge <- unlist(
  ...
  as_tibble() %>% 
  rename(
  # The variable names now begin with desc6. or dataset.
    # Jeg vil foreslå å bare bruke NACE2007 som variabelnavn
    # for næringskode
    NACE2007 = dataset.NACE2007,
    Aar = dataset.Tid,
    # Hvorfor ikke bare plukke med næringsnavn i samme slengen?
    # Jeg vil foreslå å bruke SN2007 som variabelnavn for næringsnavn
    SN2007 = `desc6.næring (SN2007)`, 
    # hvis dere ønske rtre kategorier må alder med her
    #alder = desc6.alder,
    ansatte = dataset.value
  ) %>% 
  select(Aar, NACE2007, SN2007, 
         # Hvis tre kategorier ønskes
         # alder,
         ansatte)
```
...
# Skulle dere ønske bare ønske f.eks 15 - 74 år må det spesifiseres i ApiData ovenfor
# vha. 
Nas_arbeid_wide <- Nas_arbeid_norge %>% 
  pivot_wider(
    id_cols = Aar,
    # hvis tre alderskategorier
...
```{r}
# ikke nødvendig, hentet rett fra data
# names(Nas_arbeid)[[6]] <- "Alle naeringer"
# names(Nas_arbeid)[[7]] <- "Jordbruk, skogbruk og fiske"
# names(Nas_arbeid)[[8]] <- "Bergverksdrift og utvinning"
# names(Nas_arbeid)[[9]] <- "Industri"
# names(Nas_arbeid)[[10]] <- "Elektrisitet, vann og renevasjon"
# names(Nas_arbeid)[[11]] <- "Bygge- og anleggsvirksomhet"
# names(Nas_arbeid)[[12]] <- "Varehandel, reparasjon av motorvogner"
# names(Nas_arbeid)[[13]] <- "Transport og lagring"
# names(Nas_arbeid)[[14]] <- "Overnattings- og serveringsvriksomhet"
# names(Nas_arbeid)[[15]] <- "Informasjon og kommunikasjon"
# names(Nas_arbeid)[[16]] <- "Finansiering og forsikring "
# names(Nas_arbeid)[[17]] <- "Teknisk tjenesteyting, eiendosmdrift"
# names(Nas_arbeid)[[18]] <- "Forretningsmessig tjenesteyting"
# names(Nas_arbeid)[[19]] <- "Off.adm., forsvar, sosialforsikring"
# names(Nas_arbeid)[[20]] <- "Undervisning"
# names(Nas_arbeid)[[21]] <- "Helse- og sosialtjenester"
# names(Nas_arbeid)[[22]] <- "Personlig tjenesteyting"
# names(Nas_arbeid)[[23]] <- "Uoppgitt"
```


```{r}
sum(Nas_arbeid_wide$`Alle næringer` == 0)
# Usikker på hva som er meningen med neste
#Nas_arbeid <- Nas_arbeid[Nas_arbeid$`Alle naeringer` !=0,]
```

# Sysselsatte personer etter bosted, nasjonalt

```{r}
# Bør spesifisere sysselsatte, _sys
...
```{r}
names(Nas_arbeid_raw_sys)[1] <- "desc6"
```
...
```{r}
# Unødvendig nå. Hentet rett fra data
# names(Nas_arbeid)[[6]] <- "Alle naeringer"
# names(Nas_arbeid)[[7]] <- "Jordbruk, skogbruk og fiske"
# names(Nas_arbeid)[[8]] <- "Bergverksdrift og utvinning"
# names(Nas_arbeid)[[9]] <- "Industri"
...
Bruk så samme teknikker som ovenfor for de utvalgte kommunene

```{r}
Tabell1_raw <- ApiData(
  urlToData= "07984",
  # Pick data for kommuner in knr
