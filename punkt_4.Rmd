...
Tab_03321_00_20 <- ApiData(
  urlToData = "http://data.ssb.no/api/v0/no/table/03321",
  ArbstedKomm = knr, 
  ContentsCode = "Sysselsatte",
  Bokommuen = knr,
...
```{r}
library(readxl)
Tab_03321_a_og_b <- read_excel("Data/Tab_03321_a_og_b.xlsx", 
...
Tab_03321_a_og_b %>%
  ...
  filter(
    b_knavn_new %in% c("Tysvær", "Karmøy", "Sauda")
    ) %>%
...
Vi ønsker pedlere dvs. dem som jobber i en annen kommune enn der de bor. Trenger altså å filtrere bort alle dem som har a_knr == b_knr, så gjennstår vel å gruppere mht. aar og a_knr så har vi dem som pendler inn til hver kommune.

For dem som pendler ut vil vi også filtrere bort a_knr==b_knr. Her grupperer vi mht. aar og b_knr og vil ha antallet som pendler ut fra en kommune.

```{r}
# pendler inn
tmp_pi <- Tab_03321_00_20 %>% 
  filter(!a_knr == b_knr) %>% 
  group_by(aar, a_knr) %>% 
  summarise(pendler_inn = sum())
```

```{r}
#pendler_ut
tmp_pu <- Tab_03321_00_20 %>% 
  filter(!a_knr == b_knr) %>% 
  group_by(aar, b_knr) %>% 
  summarise(pendler_ut = sum())
```

```{r}
#arbeidsplasser
tmp_arb <- Tab_03321_00_20 %>% 
  group_by(aar, a_knr) %>% 
  summarise(arbeidsplasser = sum())
```

```{r}
#bosatte
tmp_bo <- Tab_03321_00_20 %>% 
  group_by(aar, b_knr) %>% 
  summarise(bosatte = sum())
```
